...
$(document).ready(function(){
	...
	dateGreaterThanCur = "Start Time/End Time cannot be greater than or equal to  "+$("#dateHidden").val();
	startTime();
...
function addSecondsTonextPunch(th){
	...
	if(checkValidateDate(end_time) && checkValidateDate(old_time)){
		...
		if(next_date.length>0 && next_date.hasClass("DeletedRow")==false && end_time > old_time){
			...
			if(checkValidateDate(start_date_next)){
				...
				if(addSecond){
					...
					var errorTmpl = "<div class=\"TblInputFieldMessage updatedError\"><div class=\"MessageArrow\"></div>This punch has been updated to avoid overlapping punches.</div>";
					var updated = next_date.find('.start_time_input').closest('td').append(errorTmpl);
					updated.find('.TblInputFieldMessage.updatedError').delay(5000).fadeOut();
...
function getOverlapSessionFromDB(result){
    ...
    if( $(this).closest('.activities_rows').find('#start_time_lastrow').length==0){
    	...
    	var isCurrentRowOverlaps = isCurrentRowOverlap(curStartdate,curEndDate,result[4]);
        var showMSG = $(this); 
        if((result[2]=='1' || result[2]==1) && isCurrentRowOverlaps){
        ...
        if((result[3]=='1' || result[3]==1) && isCurrentRowOverlaps){
        	var errorMsg = errorTmpl.replace("{errorMsg}",timepunchOverlapErrror);
            showMSG.closest('td').append(errorMsg);
            $($('.end_time_input').get(firstIndex)).closest('td').find('.datetimeField').addClass('GridErrorField');
        }
        if(((result[2]=='0' || result[2]==0) && (result[3]=='0' || result[3]==0)) && isCurrentRowOverlaps){
            var errorMsg = errorTmpl.replace("{errorMsg}",timepunchOverlapErrror);
            showMSG.closest('td').append(errorMsg);
...
function isCurrentRowOverlap(start_time,end_time,overlap_from_db){
	var activities = overlap_from_db.split(";");
	for(var i=0;i<activities.length;i++){
		var dates = activities[i].split("-");
		var startTime = new Date(dates[0]);
		var endTime = new Date(dates[1]);
		 if((start_time<=startTime && end_time >=startTime)||(start_time<=endTime && end_time >=endTime)){
			 return true;
		 }
		 if((startTime <= start_time && start_time <= endTime)||(startTime <= end_time && end_time <= endTime)){
			 return true;
		 }
	}
}
...
function isTimeSessionValid(action){
...
function isErrorExist(e,action){
	
	var errorExist  = false;
    e.preventDefault();
    $('.errorsDiv').hide();
    $('.errorMainUl').html("");
    var timepunchesvalid = validateTimePunches();        
    if(!timepunchesvalid){
        $('.errorMainUl').append("<li>Invalid Time Punches</li>");
         
        errorExist = true;
    }else{
        var timeSessionValid =  isTimeSessionValid(action);
                      
        if(timeSessionValid!=null){
            
            getOverlapSessionFromDB(timeSessionValid);
            
            if(timeSessionValid[1] == "01/01/1970 05:00:01 AM"){
            	$('.errorMainUl').append("<li>Time Keeping Session Overlaps With a Running Time Keeping Session. " +
                 		"</li>");
            }else{
            	/*var activities = timeSessionValid[2].split(";");
            	var str = "";
            	for (i = 0; i < activities.length; i++) { 
            		str+=activities[i];
            	}*/
             /*$('.errorMainUl').append("<li>Time keeping session overlaps with existing time keeping session. " +
             		"Punches in this session cannot be before the following punch.<br><table><tr><th class='error-table'>Start Time</th><th class='error-table'>End Time</th></tr>"+timeSessionValid[0]+"</table>"+
             		
             		"<br>Also please see the punches in the previous time keeping sessions below that overlaps with the current time keeping session punches highlighted in red in the Time Keeping Session panel. " +
             				"<br><table><tr><th class='error-table'>Start Time</th><th class='error-table'>End Time</th></tr>"+timeSessionValid[1]+"</table></li>");*/
            	$('.errorMainUl').append("<li>Please see the punches that overlap in the existing time keeping session below. " +
                 				"<br><table><tr><th class='error-table'>Start Time</th><th class='error-table'>End Time</th></tr>"+timeSessionValid[1]+"</table></li>");
            }
...
function showHideRowDeleteButton(){
	if($('.activities_rows').length==1){
		$('.delete').hide();
	}else{
		$('.delete').show();
	}
