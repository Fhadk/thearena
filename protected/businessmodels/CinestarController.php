...
//          $subscribe->name=$_REQUEST['name'];
    ...
    public function actionShowTiming() {
        ...
        $locationid = 1; //also set it from getmovielist method remember it...
        for ($i = 0; $i < count($movielist); $i++) {
            $showtimings = $cinestarModel->getShowTimings($movielist[$i][0]['movie_name'], $locationid);
    ...
    public function actionIndex() {
        ...
        if (count($movielist[0]) > 0) {

            //echo(count($movielist));
            //echo("<br>");

            for ($i = 0; $i < count($movielist); $i++) {
                //echo($movielist[$i][0]['movie_name']);
                ///echo("<br>");
                $showtimings = $cinestarModel->getShowTimings($movielist[$i][0]['movie_name'], $locationid);
                $movielist[$i]['showtiming'] = $showtimings;
            }
...
//		$this->render('index',array('slidersequence'=>$slidersequence,'movielist'=>$movielist,'upcomingmovies'=>$upcomingmovies,'dateclass'=>date('Y-m-d'),'locations'=>$locations,'movieCombo'=>$movieCombo));
        $this->render('home', array('slidersequence' => $slidersequence, 'movielist' => $movielist, 'upcomingmovies' => $upcomingmovies, 'dateclass' => date('Y-m-d'), 'locations' => $locations, 'movieCombo' => $movieCombo));
    }
    ...
    public function actionDineIn() {
        ...
        $pindisnacks = $adminModel->getAllMenus(1, 1, 1);
        $pindidrinks = $adminModel->getAllMenus(1, 2, 1);
        $pindimeals = $adminModel->getAllMenus(1, 3, 1);
        $pinditreats = $adminModel->getAllMenus(1, 4, 1);
        $pindicombos = $adminModel->getAllMenus(1, 5, 1);

        $lhrsnacks = $adminModel->getAllMenus(1, 1, 2);
        $lhrdrinks = $adminModel->getAllMenus(1, 2, 2);
        $lhrmeals = $adminModel->getAllMenus(1, 3, 2);
        $lhrtreats = $adminModel->getAllMenus(1, 4, 2);
        $lhrcombos = $adminModel->getAllMenus(1, 5, 2);

        $kchsnacks = $adminModel->getAllMenus(1, 1, 2);
        $kchdrinks = $adminModel->getAllMenus(1, 2, 2);
        $kchmeals = $adminModel->getAllMenus(1, 3, 2);
        $kchtreats = $adminModel->getAllMenus(1, 4, 2);
        $kchcombos = $adminModel->getAllMenus(1, 5, 2);


        $this->render('/cinestar/dine_in', array('slidersequence' => $results, 'pindisnacks' => $pindisnacks,
            'pindidrinks' => $pindidrinks, 'pindimeals' => $pindimeals, 'pinditreats' => $pinditreats,
            'pindicombos' => $pindicombos, 'lhrsnacks' => $lhrsnacks, '$lhrdrinks' => $lhrdrinks,
            'lhrmeals' => $lhrmeals, 'lhrtreats' => $lhrtreats, 'lhrcombos' => $lhrcombos,
            'kchsnacks' => $kchsnacks, 'kchdrinks' => $kchdrinks, 'kchmeals' => $kchmeals,
            'kchtreats' => $kchtreats, 'kchcombos' => $kchcombos));
    }
...
//Set TCP port to connect to 
        ...
        $mail->FromName = "Cinestar.";

        $mail->addAddress("info@cinestar.com.pk", "Cinestar Team");
