...
class Showtiming extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('movie_id, location_id, show_order, screen',  'numerical', 'integerOnly'=>true),
			array('movies_name', 'length', 'max'=>1000),
			array('showtime_day', 'length', 'max'=>100),
			array('showdateetime', 'safe'),
			//array('screen', 'numerical', 'integerOnly'=>true),

			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, movies_name, showtime_day, showdateetime, movie_id, location_id, show_order, screen', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('screen',$this->screen);
