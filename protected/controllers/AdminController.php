...
//                            ), $criteria);  
        ...
        if (isset($dbtargetfolder) && $dbtargetfolder != '') {
            $dbtargetfolder;
        } else {
            $dbtargetfolder = '';
        }
        if (isset($dbtargetfolderlistimg) && $dbtargetfolderlistimg != '') {
            $dbtargetfolderlistimg;
        } else {
            $dbtargetfolderlistimg = '';
        }
    ...
    /* Dine In Slider Start */

    public function actionAddDineInSlider() {

        $this->layout = '/layouts/adminlayout';
        $this->render('/admin/add-new-dine_in_slider');
    }
...
// echo $dbtargetfolder; exit;
        ...
        $sliderpath = $dbtargetfolder;
        $status = $_REQUEST['status'];

        Yii::app()->db->createCommand("insert into dine_in_slider VALUES(null, '$title', '$description', '$sliderpath', '$status')")->query();


        $this->actionDineInList();
    }

    public function actionEditDineInSliderList() {

        $adminModel = new adminModel();
        $results = $adminModel->getEditableSlider($_POST['hiddenrowid']);
        $this->layout = '/layouts/adminlayout';
        Yii::app()->session['hiddensliderimgid'] = $_POST['hiddenrowid'];
        $this->render('/admin/edit-dine-in-list', array('results' => $results));
    }

    public function actionDineInList() {

        $adminModel = new adminModel();

        $results = $adminModel->getAllDineInSliders();
        if (count($results > 0)) {
            $this->layout = '/layouts/adminlayout';

            $this->render('/admin/dineInslider-list', array('results' => $results));
        }
    }
...
//                        $targetfolder = substr(Yii::app()->basePath,0,-10) . "\\cinestarimages\\";
                    ...
                    $sid = Yii::app()->session['hiddensliderimgid'];

                    Yii::app()->db->createCommand("update dine_in_slider set sliderpath = '$dbtargetfolder' where id = '$sid'")->query();
                }
            }
        }
        $title = $_REQUEST['title'];
        ...
        Yii::app()->db->createCommand("update dine_in_slider set "
                . "title = '$title', description = '$description', "
                . "status = '$status' where id = '$sid'")->query();
...
//		}
    ...
    public function actionDeleteDineInSlider() {

        $adminmodel = new adminModel();
        if (isset($_GET['id'])) {
            $adminmodel->deleteDineInSlider($_GET['id']);
        }
        $adminModel = new adminModel();
    ...
    public function actionDineInMenuList() {

        $adminModel = new adminModel();

        $results = $adminModel->getAllMenus();

        if (count($results > 0)) {
            $this->layout = '/layouts/adminlayout';
            $this->render('/admin/menu-list', array('results' => $results));
        }
    }

    public function actionAddDineInMenu() {

        $this->layout = '/layouts/adminlayout';
        $this->render('/admin/add-menu');
    }
...
// echo $dbtargetfolder; exit;
        ...
        Yii::app()->db->createCommand("insert into dine_in_menu "
                . "VALUES(null, '$title', '$description', '$price', '$sliderpath', "
                . "'$city', '$menu_category', '$status')")->query();


        $this->actionDineInMenuList();
    }

    public function actionDeleteDineInMenu() {

        $adminmodel = new adminModel();
        if (isset($_GET['id'])) {
            $adminmodel->deleteDineInMenu($_GET['id']);
        }
        $adminModel = new adminModel();
    ...
    public function actionEditDineInMenu() {

        $adminModel = new adminModel();
        $results = $adminModel->getEditableDineInMenu($_POST['hiddenrowid']);
        $this->layout = '/layouts/adminlayout';
        Yii::app()->session['hiddendineinid'] = $_POST['hiddenrowid'];
        $this->render('/admin/edit-menu', array('results' => $results));
    }
...
//                        $targetfolder = substr(Yii::app()->basePath,0,-10) . "\\cinestarimages\\";
        ...
        $menu_category = $_REQUEST['menu_category'];
        $status = $active;
        $sid = Yii::app()->session['hiddendineinid'];

        Yii::app()->db->createCommand("update dine_in_menu set "
                . "title = '$title', description = '$description', "
                . "price = '$price', city = '$city', menu_category = '$menu_category', "
                . "status = '$status' where id = '$sid'")->query();
...
//                $targetfolder=yii::app->bas;
        ...
        $upcoming->external_url = $_POST['imdblink'];
...
//                $targetfolder=yii::app->bas;
                ...
                if ($targetfolder == '') {
                    ...
                    UpcomingMovies::model()->updateAll(array('movie_title' => $_POST['movietitle'], 'external_url' => $_POST['imdblink']
                            ), $criteria);
                } else {
                    $criteria = new CDbCriteria();
                    $criteria->addCondition("id=" . $_POST['hiddenrowid']);
    ...
    public function actionEditBrandPartner() {

        $adminmodel = new adminModel();
        $targetfolder = '';
        if (isset($_POST['hiddenrowid']) && isset($_POST['title'])) {
            if ($_FILES['photo']) {
                ...
                if ($targetfolder == '') {
                    $criteria = new CDbCriteria();
                    $criteria->addCondition("id=" . $_POST['hiddenrowid']);
                    BrandPartner::model()->updateAll(array('title' => $_POST['title']
                            ), $criteria);
                } else {
                    $criteria = new CDbCriteria();
                    $criteria->addCondition("id=" . $_POST['hiddenrowid']);
        ...
        if (isset($_POST['hiddenrowid'])) {
            $results = $adminmodel->getEditBrandPartnerList($_POST['hiddenrowid']);
            $this->layout = '/layouts/adminlayout';
            $this->render('/admin/editbrandpartner', array('results' => $results));
        }
    }
